...
an edge-execution term; both abstract reads are recorded in this file under "Flagged for
abstract review" below (`research/source-validation-log.md` is generated by
`tools/build-source-registry.py` and is not hand-edited).
...
- `S37-farsa-2026-gpu-riscv`: gains `neuromorphic` alongside its existing GPU, RISC-V, and
  ...
  compare GPU, RISC-V, FPGA, ASIC, and dedicated neuromorphic hardware roles", and the
...
- `S10-eshraghian-2023-training-snns`: H axis `mentioned`, "Specialized hardware motivates
  SNNs, without platform taxonomy", a training tutorial in which hardware is motivation only.
...
- `S19-xie-2024-compression`: H axis `mentioned`, "Neuromorphic chips motivate efficient
  DSNNs, without a platform taxonomy", named as motivation and explicitly not surveyed.
