...
Each line covers one evidence-papers.json record, giving its id, target_kind, qualifier, the field text that decided it, and the rule number from the list below. Records split from a single source paper are listed as two lines.

## Rules, applied in order

1b. An event-domain neural accelerator whose documented path runs a converted network as sparse events and exposes no spiking neuron model over T is `event NPU` (Akida; spec amendment 1). It is neither the dense `NPU` of rules 2 and 5 nor the `neuromorphic` silicon of rule 1.
1. `target_hardware` names a fabricated spiking chip (Loihi 1 or 2, SpiNNaker, TrueNorth, Tianjic, Speck, DYNAP-CNN, DYNAP-SE or SE2, BrainScaleS, Xylo, Akida, Lynxi KA200, MorphIC, Innatera) → `neuromorphic`, qualifier the chip and generation as the record names it (Akida is taken by rule 1b first).
2. `target_hardware` names NorthPole → `NPU`, qualifier "IBM NorthPole, non-spiking digital accelerator" (ruling, 12.6 already states this; the record is corrected here).
3. `target_hardware` names an FPGA design that the paper itself built and measured as the target (SyncNN, Cerebron, Firefly and any other FPGA accelerator) → `FPGA`, qualifier the board or family the record names.
4. `target_hardware` names an RTL, synthesis or cycle model without fabricated silicon (APEX, NeuroFlex, SENECA, PASCAL, XpikeFormer and any record graded E4 for that reason) or a chip's software simulator → `simulator`, qualifier the model or simulator name.
5. `target_hardware` or the notes say the run happened on a CPU, GPU, MCU, RISC-V core or Jetson as the reported target → the matching conventional word, qualifier the part as named (ruling of 2026-09-17, a Jetson module is `GPU`, and a dense NPU other than NorthPole, such as the Coral Edge TPU, an Ethos-U class accelerator or a phone NPU, is `NPU` by this rule).
6. Otherwise (`target_hardware` "none", evidence E5, algorithm-only) → `"none"`, qualifier `""`.
...
- `akida` is **event NPU** (spec amendment 1, rule 1b), basis `data/platform-capabilities.json` akida-1 `time_termination`, which reads "The standard converted path is a single event-domain pass. It does not expose the conversion literature's global simulation length T."; `neural_code_payload`, which reads "Akida events carry low-bit activation information."
