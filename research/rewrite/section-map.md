...
| 5. Where an SNN Runs | `where-an-snn-runs` | `three-targets-one-network`, `conventional-platforms`, `simulated-emulated-neuromorphic`, `synchronous-digital`, `asynchronous-digital`, `programmable-manycore`, `analog-mixed-signal`, `fpga-asic`, `conventional-emerging`, `targets-referrals` | Name the three targets and compare the paths between them, then describe each execution family before any network is fitted to it. Use platform examples only with generation, toolchain version, capability status, and evidence limits. |
