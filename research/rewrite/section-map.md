...
| 5. Where an SNN Runs | `where-an-snn-runs` | `evidence-guide`, `three-targets-one-network`, `conventional-platforms`, `simulated-emulated-neuromorphic`, `synchronous-digital`, `asynchronous-digital`, `programmable-manycore`, `analog-mixed-signal`, `fpga-asic`, `conventional-emerging`, `targets-referrals` | Define the E1 through E5 grade every deployment claim carries, name the three targets and compare the paths between them, then describe each execution family before any network is fitted to it. Use platform examples only with generation, toolchain version, capability status, and evidence limits. |
...
Folded anchors kept inline are physical-neuromorphic-silicon (5.4), hardware-execution-models (5.5), input-and-readout and parameter-mapping and architecture-preparation (8.3), classical-failure-cases (8.5), finite-time-error-low-latency (8.6), residual-reset and unevenness-timing and encoding-readout-error (8.7), low-t-design-problem (8.8), advanced-conversion-methods and potential-reset-calibration (8.9), temporal-allocation (8.10), alternative-code-conversion (8.11), hybrid-fine-tuning (8.12), what-the-evidence-shows (13.6).
