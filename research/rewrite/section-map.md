...
|---|---|---|---|
| 1. Introduction | `introduction` | `existing-surveys`, `survey-contributions`, `evidence-guide`, `survey-roadmap` | Establish the deployment question, scope, family-level prior-work synthesis, E1 through E5 guide, motivating two-population question, contributions, and reading order. Keep the complete 37-work matrix. |
...
| 5. Where an SNN Runs | `where-an-snn-runs` | `three-targets-one-network`, `conventional-platforms`, `simulated-emulated-neuromorphic`, `synchronous-digital`, `asynchronous-digital`, `programmable-manycore`, `analog-mixed-signal`, `fpga-asic`, `conventional-emerging`, `targets-referrals` | Name the three targets and compare the paths between them, then compare execution families through contract fields. Use platform examples only with generation, toolchain version, capability status, and evidence limits. |
| 6. The Two Branches | `learning-paradigms-two-branches` | `spike-learning-problem`, `direct-conversion-hybrid`, `branch-convergence` | Introduce direct training, conversion, and hybrid routes as a fork whose outputs must satisfy one deployable contract. The claim-level evidence classes E1 through E5 are defined once, in Section 1.3, and applied to every deployment claim throughout. |
...
Folded anchors kept inline are physical-neuromorphic-silicon (5.3), hardware-execution-models (5.4), input-and-readout and parameter-mapping and architecture-preparation (8.3), classical-failure-cases (8.5), finite-time-error-low-latency (8.6), residual-reset and unevenness-timing and encoding-readout-error (8.7), low-t-design-problem (8.8), advanced-conversion-methods and potential-reset-calibration (8.9), temporal-allocation (8.10), alternative-code-conversion (8.11), hybrid-fine-tuning (8.12), what-the-evidence-shows (13.6).
