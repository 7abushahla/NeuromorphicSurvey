...
| 14. Complete Deployment Routes | `complete-deployment-routes` | `route-definition`, `physical-routes`, `approximate-transformed-routes`, `blocked-unexercised-routes`, `route-selection` | Trace named routes edge by edge. Do not infer a route from compatible-looking nodes. Place the interactive map after the prose synthesis. |
