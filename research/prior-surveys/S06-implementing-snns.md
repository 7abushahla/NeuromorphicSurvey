...
|---|---|---|---|
| A | none | IF/LIF serve only as introductory background. | Sec. 1, Fig. 1 |
| B | none | ISI coding is used to illustrate timing distortion, not surveyed as encoding taxonomy. | Sec. 2.1 |
| C | none | Learning methods are contextual rather than reviewed. | Sec. 2, platform-based design |
| D | none | ANN-to-SNN conversion is not a review category. | Secs. 2-3 |
| E | none | General simulators are named as application-level inputs, not compared as frameworks. | Sec. 2.1 |
...
| K | none | Application examples explain mapping but no application-results survey is organized. | Secs. 2-3 |
