...
def load_sections():
    ...
    for key, suffix in SUFFIX.items():
        ...
        sections[key] = (int(matches[0]["display_number"]), str(matches[0]["title"]))
...
class Canvas:
    ...
    def label(self, key):
        number, title = self.sections[key]
    ...
    def bucket_box(self, key, x, y, w, buckets):
        ...
        for index, name in enumerate(BUCKET_ORDER):
            ...
            parts.append(
                ...
                f'text-anchor="middle" fill="{b["ink"]}" font-size="{BUCKET_PX}">{html.escape(name)}</text></g>'
            )
        parts.append("</g>")
